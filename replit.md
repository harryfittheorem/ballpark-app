...
- **Done (v0.3 Steps 3.9 + 3.10, 2026-05-05):** Home Upcoming Session card pulls real bookings; Me tab Bookings list (upcoming/past split) shipped (Tasks #35 / #36 / #41 / #42).
- **Done (v0.3 Step 3.11, 2026-05-05):** Family `primary_location_id` defaulting + Date picker fix. New migration `20260505070000_v03_default_primary_location.sql` updates `handle_new_user` to resolve the tenant's first location (alphabetical) and insert it on the new families row, and backfills existing NULL rows with the same per-tenant default — fixing the "Date picker stuck on spinner" bug whose root cause was every existing dev family having `primary_location_id = NULL`. AddKid screen now renders a "Home location" `PickerField` (hidden when the tenant has zero locations) defaulted to the trigger-set value; `useAddKid` updates `families.primary_location_id` first when changed, then inserts the kid. `DateSection` and `TimeSection` also gain a defensive guard so a disabled location query (`fetchStatus='idle'`) no longer pins `isPending=true` — the calendar / time grid render against the device timezone fallback. Backfill verified: all 4 dev families now point to Dallas N. (`...0101`). Types regenerated, all gates green. v0.4 will add a Me-tab "change home location" affordance.
